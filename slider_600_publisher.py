...
class ElephantRobot(object):
    ...
    def set_digital_out(self, pin_number, pin_signal):
        command = "set_digital_out(" + str(pin_number) + \
            "," + str(pin_signal) + ")\n"
        self.send_command(command)

    def set_analog_out(self, pin_number, pin_signal):
        command = "set_analog_out(" + str(pin_number) + \
            "," + str(pin_signal) + ")\n"
        self.send_command(command)
    ...
    def assign_variable(self, var_name, var_value):
        command = 'assign_variable("' + str(var_name) + \
            '",' + str(var_value) + ")\n"
        self.send_command(command)
...
def publish_real_joint_states():
    global mc
    rospy.init_node('real_join_angles_pub', anonymous=True)
    ip = rospy.get_param("~ip", "192.168.50.241")
    print(ip)
    ...
    pub_joint = rospy.Publisher(
        'real_states', Float64MultiArray, queue_size=10)
    rate111 = rospy.Rate(50)
    while not rospy.is_shutdown():
        ffloat_64_multi_array = Float64MultiArray()
        ffloat_64_multi_array.data = mc.get_angles()
        pub_joint.publish(ffloat_64_multi_array)
        rate111.sleep
